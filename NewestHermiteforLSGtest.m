...
elem = double(elem);
[elem2dof1,elem2edge,edge,bdDof,freeDof] = dof_P4Hermite(elem,node); 
...
N = size(node,1);
NT = size(elem,1);
DG_dof = [elem, (N+1:N+NT)', (N+NT+1:N+2*NT)',(N+2*NT+1:N+3*NT)',(N+3*NT+1:N+4*NT)',(N+4*NT+1:N+5*NT)',(N+5*NT+1:N+6*NT)',(N+6*NT+1:N+7*NT)'];
...
BAE = zeros(3,15);
for i = 1:15
    BAE(1,i) = 0^m3(i,1)*com_quad(m3(i,[2,3])');
    BAE(2,i) = 0^m3(i,2)*com_quad(m3(i,[1,3])');
    BAE(3,i) = 0^m3(i,3)*com_quad(m3(i,[1,2])');
end

QA = zeros(5,1);
m1 = [0,0];
m2 = [2,0;0,2;1,1;2,1;1,2];

    for i = 1:5
    QA(i,1)=com_quad((m1(1,:)+m2(i,:))');
    end
...
[ A] = combase_P4Hermite( Dl,nor,QA);
A(10:12,:) = BAE; 
A(13:15,:)=BA;

cof = inv(A);
COF((i-1)*base_num+1:i*base_num,:)=cof'; %每一行代表一个单元的�?个基的系�?
...
%RDGdof = [Relem,(RN+1:RN+RNT)',(RN+RNT+1:RN+2*RNT)',(RN+2*RNT+1:RN+3*RNT)',(RN+3*RNT+1:RN+4*RNT)',(RN+4*RNT+1:RN+5*RNT)',(RN+5*RNT+1:RN+6*RNT)',(RN+6*RNT+1:RN+7*RNT)'];
%Rint_u = Rsolut_u(RDGdof);
RNT = size(Relem,1);
...
Dy(:,14) = 2*lambda2(p,3)*lambda2(p,1)^2*Dlambda(:,2,3) + 2*lambda2(p,3)^2*lambda2(p,1)*Dlambda(:,2,1);   
    for i = 1:DG_base_num
      Cipinter(:,i)= Cipinter(:,i)+int_u(:,i).*((DG_phi(p,:)*DGcof(:,i))*ones(NT,1));
      Cipinteru1(:,i)= Cipinteru1(:,i)+int_u1(:,i).*((DG_phi(p,:)*DGcof(:,i))*ones(NT,1));
   end  
   for i = 1:base_num
      %% sigma11
       Cipinter1(:,i) = Cipinter1(:,i)+int_s(:,i).*(COF(i:base_num:end,:)*(phi(p,:)'));
      DxCipinter(:,i)= DxCipinter(:,i)+int_s(:,i).*dot(COF(i:base_num:end,:),Dx,2);
      DyCipinter(:,i)= DyCipinter(:,i)+int_s(:,i).*dot(COF(i:base_num:end,:),Dy,2);
      %%  sigma22
      Cipinters1(:,i) = Cipinters1(:,i)+int_s1(:,i).*(COF(i:base_num:end,:)*(phi(p,:)'));
      DxCipinters1(:,i)= DxCipinters1(:,i)+int_s1(:,i).*dot(COF(i:base_num:end,:),Dx,2);
      DyCipinters1(:,i)= DyCipinters1(:,i)+int_s1(:,i).*dot(COF(i:base_num:end,:),Dy,2);
      %% sigma12
      Cipinters2(:,i) = Cipinters2(:,i)+int_s2(:,i).*COF(i:base_num:end,:)*(phi(p,:)');
      DxCipinters2(:,i)= DxCipinters2(:,i)+int_s2(:,i).*dot(COF(i:base_num:end,:),Dx,2);
      DyCipinters2(:,i)= DyCipinters2(:,i)+int_s2(:,i).*dot(COF(i:base_num:end,:),Dy,2);
